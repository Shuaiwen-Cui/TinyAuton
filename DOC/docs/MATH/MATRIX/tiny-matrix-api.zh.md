...
根据计算的内存需求为矩阵分配内存的内部函数。设置`ext_buff = false`并分配`row * step`个float元素。
...
Mat - 一个`ext_buff = true`并具有指定step的矩阵视图。
...
位置(row, col)的元素访问为`data[row * step + col]`，其中step考虑了填充。
...
- `int max_minors_to_check` : 要检查的前导主子式个数。`-1`表示检查全部；`>0`表示仅检查前若干个。
