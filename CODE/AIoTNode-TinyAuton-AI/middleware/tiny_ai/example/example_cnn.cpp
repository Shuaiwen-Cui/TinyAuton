...
{
    printf("\n========================================\n");
    printf("  BatchNorm2D Demo (Signal)\n");
    printf("========================================\n");
    // Conv1D(1,8,5)+BN2D(8)+ReLU+MaxPool1D(2)  → [B,8,30]
    // Conv1D(8,16,3)+BN2D(16)+ReLU+MaxPool1D(2) → [B,16,14]
    // Flatten → [B,224] → Dense(224,32)+ReLU → Dense(32,3)+Softmax
    printf("  Conv1D(1,8,5) + BN2D(8) + ReLU + MaxPool1D(2)\n");
    printf("  Conv1D(8,16,3) + BN2D(16) + ReLU + MaxPool1D(2)\n");
    printf("  Flatten → Dense(224,32)+ReLU → Dense(32,3)+Softmax\n\n");
    ...
    {
        train_ds.shuffle((uint32_t)(epoch + 200));
        ...
        if ((epoch + 1) % 10 == 0)
            printf("  Epoch [%3d/%3d]  loss: %.6f\n",
                   epoch + 1, epochs, epoch_loss / (float)n_batches);
    }
    TINY_AI_FREE(y_batch);

    // Accuracy helper (reusable for both modes)
    auto eval_acc = [&](Dataset &ds, const char *tag, bool train_mode)
    {
        model.set_training_mode(train_mode);
        ds.reset();
        ...
        printf("  [%s] %s acc: %.2f%%\n",
               train_mode ? "train mode" : "infer mode", tag,
               100.0f * correct / (float)total);
    };

    printf("\n--- training_mode = ON (batch stats) ---\n");
    eval_acc(train_ds, "Train", true);
    eval_acc(test_ds,  "Test ", true);

    // --- Inference mode: switch to running_mean / running_var ---
    printf("\n[BN2D] training_mode = OFF  (running stats active)\n");
    printf("--- training_mode = OFF (running stats) ---\n");
    eval_acc(train_ds, "Train", false);
    eval_acc(test_ds,  "Test ", false);

    // Show accumulated channel statistics for the first conv's BN layer
    int n_show = (bn0->running_mean.size < 4) ? bn0->running_mean.size : 4;
    printf("\nBN2D[0] running_mean (first %d ch): ", n_show);
    for (int i = 0; i < n_show; i++) printf("%.4f ", bn0->running_mean.data[i]);
    printf("\nBN2D[0] running_var  (first %d ch): ", n_show);
...
void example_cnn(void)
{
    ...
    // CNN forward expects [B, 1, L], but Dataset returns [B, L].
    // We override with a lambda-style wrapper that reshapes before forwarding.
    // Since Sequential::forward operates on Tensors we reshape the batch inline.
    // (In practice the model summary already shows the required input shape.)
    ...
    // ---- BatchNorm2D demo ----
#if TINY_AI_TRAINING_ENABLED
    batchnorm2d_demo(train_ds, test_ds);
#endif
